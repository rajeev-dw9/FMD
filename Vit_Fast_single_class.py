...
class VitModelForSingleDisease(nn.Module):
    def __init__(self, use_pretrained=True, pretrained_path=None):
        ...
        self.vit = timm.create_model('vit_base_patch16_224', pretrained=use_pretrained, num_classes=1)
...
def train_model(model, train_loader, val_loader, optimizer, num_epochs=5):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (images, targets) in enumerate(t):
            ...
            t.set_postfix(loss=train_loss / (step + 1), acc=correct_preds / total_preds)
...
def main():
    ...
    model = VitModelForSingleDisease(
        use_pretrained=use_pretrained,
        pretrained_path=pretrained_model_path if use_pretrained else None
    ).to(device)
    optimizer = optim.AdamW(model.parameters(), lr=1e-5)
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=1e-4, steps_per_epoch=len(train_loader), epochs=5)
    scheduler.step()
